...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stat-value { font-size: 14.5px; color: var(--text); font-weight: 600; margin-top: 4px; word-break: break-word; font-family: var(--font-mono); }
        ...
        <div class="overview-top">
            <div class="ip-hero">
                ...
                <div class="ip-hero-body">
                    ...
                    <div class="ip-hero-value" id="ip">--</div>
                    <div class="ip-hero-meta">
                        <span id="asn">--</span><span class="sep">·</span><span id="org">--</span>
                    </div>
                </div>
            </div>
            ...
            <button class="copy-btn" id="copyIp">
                ...
                <span data-i18n="btn.copy">复制</span>
            </button>
        </div>
        <div class="overview-grid">
            <div class="stat"><div class="stat-label" data-i18n="label.city">城市</div><div class="stat-value" id="city">--</div></div>
            <div class="stat"><div class="stat-label" data-i18n="label.timezone">时区</div><div class="stat-value" id="timezone">--</div></div>
            <div class="stat"><div class="stat-label" data-i18n="label.coords">坐标</div><div class="stat-value" id="coords">--</div></div>
            <div class="stat"><div class="stat-label" data-i18n="label.postal">邮编</div><div class="stat-value" id="postal">--</div></div>
        </div>
...
(function () {
    ...
    function calculateOverallScore(mergedQuality, mergedDetail, isDC, isVPN, isProxy, isTor) {
        ...
        // 移动网络：轻微扣分（5分），移动IP稳定性较差
        if (mergedDetail && mergedDetail.is_mobile) riskScore -= 5;
        
        // 卫星网络：轻微扣分（8分），延迟高
        if (mergedDetail && mergedDetail.is_satellite) riskScore -= 8;
            ...
            if (el && el.textContent) {
                ...
                if (isReachable) {
                    ...
                    if (match) {
                        ...
                        // 优化延迟评分公式：<50ms=100分，50-200ms线性递减到80分，200-500ms递减到60分，>500ms=40分
                        var latencyScore;
                        if (latency < 50) {
                            latencyScore = 100;
                        } else if (latency < 200) {
                            latencyScore = 100 - (latency - 50) * 20 / 150; // 线性递减20分
                        } else if (latency < 500) {
                            latencyScore = 80 - (latency - 200) * 20 / 300; // 线性递减20分
                        } else if (latency < 1000) {
                            latencyScore = 60 - (latency - 500) * 20 / 500; // 继续递减
                        } else {
                            latencyScore = 30; // 超过1秒，严重问题
                        }
                        siteScores.push(latencyScore);
                    } else {
                        siteScores.push(75); // 可达但无延迟数据
                    }
        ...
        // ========== 3. 综合评分 ==========
        // 权重：IP风险75%，可达性25%（风险是主要因素）
        var finalScore = Math.round(riskScore * 0.75 + reachScore * 0.25);
    ...
    els.copyIp.addEventListener('click', function () {
        ...
        navigator.clipboard.writeText(text).then(function () {
            ...
            var span = els.copyIp.querySelector('span');
            if (span) span.textContent = t('btn.copied');
            setTimeout(function () {
                els.copyIp.classList.remove('copied');
                if (span) span.textContent = t('btn.copy');
            }, 1600);
        }).catch(function () {});
    });
