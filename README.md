...
查询并展示您的 IPv4/IPv6 地址、国内/海外连通性、以及 ASN 信息。
...
- **综合评分系统**：基于 IP 干净程度和网络可达性的智能评分（0-100分）。
...
**1. IP风险评分（满分100分，占比70%）**

从100分开始，根据以下风险因素扣分：

| 风险类型 | 扣分 | 说明 |
|---------|------|------|
| 🏢 数据中心IP (Datacenter) | -35分 | 严重风险：机房IP常被用于自动化操作 |
| 🔒 VPN | -30分 | 严重风险：匿名化工具 |
| 🌐 代理 (Proxy) | -30分 | 严重风险：流量中转节点 |
| 🧅 Tor 洋葱路由 | -40分 | 极高风险：暗网匿名网络 |
| 🤖 爬虫 (Crawler) | -25分 | 高风险：自动化工具 |
| ⚠️ 滥用记录 (Abuser) | -30分 | 高风险：历史恶意行为 |
| 🎭 匿名IP | -20分 | 中等风险：隐藏真实身份 |
| 🚫 保留IP (Bogon) | -50分 | 极严重：非法或保留地址段 |
| 📱 移动网络 (Mobile) | **0分** | ✅ 正常网络类型，不扣分 |
| 🛰️ 卫星网络 (Satellite) | **0分** | ✅ 正常网络类型，不扣分 |

> **数据源整合**：综合 `ipinfo.io`、`ipapi.is`、`IP2Location`、`Scamalytics`、`ipregistry`、`IPData`、`IPWhois`、`DB-IP`、`AbuseIPDB`、`Cloudflare` 等多个数据源，通过多数表决确保准确性。

**2. 可达性评分（满分100分，占比30%）**

检测目标网站：Google、Baidu、ChatGPT

评分规则：
- **可达基础分**：80分（只要能连通即可获得）
- **延迟加分/扣分**：
  - ≤200ms：80-100分（延迟越低分数越高）
    - 0ms → 100分
    - 100ms → 90分
    - 200ms → 80分
  - 200-500ms：80→60分（线性递减）
  - 500-1000ms：60→50分（线性递减）
  - >1000ms：40分（严重延迟但仍可达）
- **不可达**：0分
...
| 分数范围 | 等级 | 颜色 | 说明 |
|---------|------|------|------|
| 80-100分 | 优秀 | 🟢 绿色 | IP干净且网络畅通 |
| 60-79分 | 良好 | 🔵 蓝色 | 轻微风险或延迟 |
| 40-59分 | 一般 | 🟠 橙色 | 存在明显风险 |
| 0-39分 | 较差 | 🔴 红色 | 高风险或不可达 |
...
- **家宽用户**（非VPN、延迟50ms）：
  - IP风险：100分
  - 可达性：95分
  - 综合：100×0.7 + 95×0.3 = **98.5分（优秀）**

- **机房IP**（Datacenter、延迟30ms）：
  - IP风险：65分（-35分）
  - 可达性：98分
  - 综合：65×0.7 + 98×0.3 = **75分（良好）**

- **VPN用户**（VPN、延迟150ms）：
  - IP风险：70分（-30分）
  - 可达性：87分
  - 综合：70×0.7 + 87×0.3 = **75分（良好）**
